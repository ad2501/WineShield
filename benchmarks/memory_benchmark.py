#!/usr/bin/env python3
"""
memory_benchmark.py — WineShield Memory Overhead Benchmark

Measures the **resident set size (RSS) in kilobytes** that each
security-layer configuration adds to a standard Wine workload.  This
captures the memory footprint of the monitoring layers (seccomp,
network guard, behavior analyzer) while running a Windows application
under Wine.
...
1. For each of the 5 configurations (baseline, seccomp_only,
   ...
   d. Parse the ``Maximum resident set size (kbytes)`` field from the
      time output
   e. Repeat 3 times
...
-----
    ...
    python3 benchmarks/memory_benchmark.py --runtime 15 --output report.json

Output
------
- Printed summary table to stdout
- Optional JSON file (default: memory_benchmark_<timestamp>.json)
"""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="WineShield Memory Overhead Benchmark",
    ...
    # ── Pre-flight checks ──────────────────────────────────────────
    print("=" * 72)
    print("  WineShield — Memory Overhead Benchmark")
    print("=" * 72)
    print()
    ...
    reporter = ReportWriter(
        ...
        metric_unit="KB",
        description="Maximum resident set size (RSS) during Wine workload execution",
    )
    ...
    for cfg in CONFIGURATIONS:
        ...
        for i in range(1, args.iterations + 1):
            ...
            try:
                ...
                if rss is not None:
                    rss_mb = rss / 1024.0
                    print(f"RSS = {rss_mb:.1f} MB ({rss:,} KB)", end="")
                else:
                    print(f"RSS = N/A", end="")
        ...
        if stats["mean"] is not None:
            mean_mb = stats["mean"] / 1024.0
            std_mb = stats["stddev"] / 1024.0
            min_mb = stats["min"] / 1024.0
            max_mb = stats["max"] / 1024.0
            print(f"     ─> Avg RSS: {mean_mb:.1f} MB ± {std_mb:.1f} MB "
                  f"(range: {min_mb:.1f}–{max_mb:.1f} MB)")
        else:
            print(f"     ─> Avg RSS: N/A (all runs failed)")
        print()
    ...
    if args.output:
        ...
        out_path = f"memory_benchmark_{ts}.json"
